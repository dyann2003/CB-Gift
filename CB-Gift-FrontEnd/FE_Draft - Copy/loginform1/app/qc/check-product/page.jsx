...
export default function CheckProduct() {
  ...
  const loadOrders = async () => {
    try {
      ...
      const transformedData = data.map((order) => {
        ...
        const customerFiles = order.details.map((detail) => ({ /* ... keep if needed ... */ }));
        const designFiles = order.details
          .filter((detail) => detail.linkFileDesign)
          .map((detail) => ({ /* ... keep if needed ... */ }));
        ...
        return {
          ...
          dateManufactured: new Date(order.orderDate).toLocaleDateString("vi-VN"),
          status: order.statusOderName, // String status name from API
          statusOrderId: order.statusOrder, // Numeric status ID from API
          customerInfo: {
          ...
          customerFiles: customerFiles, // Keep data even if not displayed
          designFiles: designFiles,     // Keep data even if not displayed
          productImages: [],
        };
      });
      ...
      setError("Could not load orders. Please check the API connection and try again.");
  ...
  const filteredOrders = allOrders.filter((order) => {
    ...
    const customerMatch = order.customer?.toLowerCase().includes(lowerSearchTerm);
    ...
    const matchesFilter = filterStatus === "all" || orderStatusString === filterStatus;
  ...
  const handleManualOrderSearch = () => {
    ...
    const foundOrder = allOrders.find(
      (order) => order.id?.toString().toLowerCase() === manualOrderId.trim().toLowerCase() ||
  ...
  const handleApprove = (orderId) => {
    setConfirmMessage(`Are you sure you want to approve order ${selectedOrder?.code || orderId} for shipping?`);

    setConfirmAction(() => async () => {
      console.log(`Attempting to approve order ${orderId}...`);
      try {
        setLoading(true); // Indicate loading state during API call
        const response = await fetch(`https://localhost:7015/api/Order/${orderId}/approve-shipping`, {
          method: 'PUT',
          headers: {
  ...
  const handleRejectConfirm = async () => { // Make async for potential API call
    if (rejectReason.trim() && selectedOrder) {
      const orderId = selectedOrder.id;
      console.log(`Attempting to reject order ${orderId}. Reason: ${rejectReason}...`);
      try {
        setLoading(true);
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="flex gap-2 items-center">
                <Input
                  ...
                  className="flex-1"
                />
                <Button onClick={handleManualOrderSearch} variant="outline" size="sm">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {loading && (
                    <tr><td colSpan="6" className="text-center px-6 py-4 text-gray-500">Loading orders...</td></tr>
                  )}
                  {error && (
                    <tr><td colSpan="6" className="text-center px-6 py-4 text-red-600">{error}</td></tr>
                  )}
                  {!loading && !error && filteredOrders.length === 0 && (
                    <tr><td colSpan="6" className="text-center px-6 py-4 text-gray-500">No orders found matching your criteria.</td></tr>
                  )}
                  {!loading && !error && filteredOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600 max-w-xs truncate" title={order.products?.map((p) => p.name).join(", ")}>
                        {order.products?.map((p) => p.name).join(", ") || 'N/A'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{order.staff}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{order.dateManufactured}</td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 transition-opacity duration-300">
          <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale flex flex-col">
                ...
                <div className="sticky top-0 bg-white px-6 py-4 border-b border-gray-200 flex justify-between items-center z-10 flex-shrink-0">
                  <h2 className="text-lg font-semibold text-gray-800">Product Inspection - Order {selectedOrder.code}</h2>
                    ...
                    <h3 className="font-semibold text-base mb-3 text-gray-700">Customer Information</h3>
                       <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                         <div>
                           ...
                           <p className="font-medium text-gray-800">{selectedOrder.customerInfo?.name || 'N/A'}</p>
                         </div>
                         <div>
                           <Label className="text-xs text-gray-500">Phone</Label>
                           <p className="font-medium text-gray-800">{selectedOrder.customerInfo?.phone || 'N/A'}</p>
                         </div>
                         <div className="col-span-2">
                           <Label className="text-xs text-gray-500">Email</Label>
                           <p className="font-medium text-gray-800 truncate">{selectedOrder.customerInfo?.email || 'N/A'}</p>
                         </div>
                         <div className="col-span-2">
                           <Label className="text-xs text-gray-500">Address</Label>
                           <p className="font-medium text-gray-800">
                             {`${selectedOrder.customerInfo?.address || ''}${selectedOrder.customerInfo?.city ? `, ${selectedOrder.customerInfo.city}` : ''}${selectedOrder.customerInfo?.state ? `, ${selectedOrder.customerInfo.state}` : ''}${selectedOrder.customerInfo?.zipcode ? ` ${selectedOrder.customerInfo.zipcode}` : ''}${selectedOrder.customerInfo?.country ? `, ${selectedOrder.customerInfo.country}` : ''}` || 'N/A'}
                           </p>
                         </div>
                       </div>
                  </div>
                      ...
                      {selectedOrder.products?.map((product, index) => (
                        <div key={index} className="bg-white p-3 rounded border border-gray-200 shadow-sm">
                          <div className="flex justify-between items-center mb-2">
                            <p className="font-medium text-sm text-gray-800 flex-1 mr-2 truncate" title={product.name}>{product.name || 'Unnamed Product'}</p>
                            <Button
                              variant="outline"
                              size="sm"
                              ...
                              onClick={() => {
                                if (product.orderDetailID) {
                                  ...
                                  router.push(`/qc/order-detail/${product.orderDetailID}`);
                                } else {
                          ...
                          <div className="grid grid-cols-4 gap-x-2 text-xs">
                            ...
                            <div><Label className="text-gray-500">Status:</Label><Badge variant={getStatusBadgeVariant(product.status)} className="text-xs px-1.5 py-0.5">{product.status || 'Unknown'}</Badge></div>
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm pt-3 border-t border-blue-100">
                       <div><Label className="text-xs text-gray-500">Staff Assigned</Label><p className="font-medium text-gray-800">{selectedOrder.staff || 'N/A'}</p></div>
                       <div><Label className="text-xs text-gray-500">Order Date</Label><p className="font-medium text-gray-800">{selectedOrder.dateManufactured || 'N/A'}</p></div>
                       {selectedOrder.productDetails?.specialInstructions && (<div className="col-span-2 mt-1"><Label className="text-xs text-gray-500">Notes</Label><p className="font-medium text-gray-800 text-sm bg-yellow-100 border border-yellow-200 p-2 rounded mt-1">{selectedOrder.productDetails.specialInstructions}</p></div>)}
                    </div>
                  </div>
                </div> {/* End Modal Body */}
                ...
                <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-lg flex-shrink-0">
                  {(() => {
                    ...
                    const isAlreadyApprovedOrBeyond = selectedOrder.statusOrderId >= 12;
                    ...
                    if (isAlreadyApprovedOrBeyond) {
                      return (
                        <>
                          ...
                          <Button className="bg-gray-400 text-white px-5 cursor-not-allowed" disabled={true} title="This order has already been approved or shipped.">
                      ...
                      const canApprove = selectedOrder.products?.length > 0 &&
                      ...
                      return (
                        <>
                          ...
                          <Button
                            ...
                            title={!canApprove ? "All products must have status QC_DONE to approve" : "Approve order for shipping"}
                          >
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[60] transition-opacity duration-300">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale">
                 <style jsx global>{`
                   ...
                   .animate-fade-in-scale { animation: fade-in-scale 0.2s ease-out forwards; }
                 `}</style>
                 <h3 className="text-lg font-semibold mb-4 text-gray-800">Reject Product</h3>
                 <div className="space-y-4">
                   <div>
                     <Label htmlFor="reject-reason" className="text-sm font-medium text-gray-700">Reason for rejection <span className="text-red-500">*</span></Label>
                     <Textarea
                       id="reject-reason"
                     ...
                     />
                     {!rejectReason.trim() && <p className="text-xs text-red-500 mt-1">Reason cannot be empty.</p>}
                   </div>
                 </div>
                 <div className="flex gap-3 justify-end mt-6">
                   <Button variant="outline" onClick={() => { setShowRejectDialog(false); setRejectReason(""); }}>Cancel</Button>
                   <Button onClick={handleRejectConfirm} className="bg-red-600 hover:bg-red-700 text-white disabled:opacity-50" disabled={!rejectReason.trim() || loading}> {/* Disable while loading */}
                     Confirm Rejection
                   </Button>
                 </div>
           </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[60] transition-opacity duration-300">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale">
                ...
                <div className="flex gap-3 justify-end">
                  <Button variant="outline" onClick={() => setShowConfirmDialog(false)} disabled={loading}>Cancel</Button> {/* Disable while loading */}
                  <Button onClick={() => { if (confirmAction) confirmAction(); }} className="bg-blue-600 hover:bg-blue-700 text-white" disabled={loading}>Confirm</Button> {/* Disable while loading */}
                </div>
            </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[60] transition-opacity duration-300">
           <div className="bg-white rounded-lg shadow-xl max-w-lg w-full transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale flex flex-col max-h-[85vh]">
                ...
                <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 flex-shrink-0">
                  <h3 className="text-lg font-medium text-gray-800 text-center flex-grow">Product Details</h3>
                  <Button variant="ghost" size="icon" onClick={() => setViewingProduct(null)} className="rounded-full text-gray-500 hover:bg-gray-100 -mr-2">
                    <X className="h-5 w-5" />
                  </Button>
                </div>
                <div className="p-6 space-y-5 overflow-y-auto">
                  <div className="text-center mb-4">
                    <p className="text-xl font-semibold text-gray-900">{viewingProduct.name || 'N/A'}</p>
                  </div>
                  <dl className="border-t border-gray-200 pt-4">
                    <div className="grid grid-cols-3 gap-x-4 gap-y-4 text-sm">
                      <div className="border-b border-gray-100 pb-2">
                        ...
                        <dd className="mt-1 text-gray-900 font-medium">{viewingProduct.quantity ?? '?'}</dd>
                      </div>
                      <div className="border-b border-gray-100 pb-2">
                        <dt className="text-xs font-medium text-gray-500 uppercase tracking-wider">Size</dt>
                        <dd className="mt-1 text-gray-900 font-medium">{viewingProduct.size || '-'}</dd>
                      </div>
                      <div className="border-b border-gray-100 pb-2">
                        <dt className="text-xs font-medium text-gray-500 uppercase tracking-wider">Accessory</dt>
                        <dd className="mt-1 text-gray-900 font-medium">{viewingProduct.accessory || '-'}</dd>
                      </div>
                    </div>
                  </dl>
                  <div className="pt-4">
                    <h4 className="text-sm font-semibold text-gray-700 mb-2">Note / Instructions</h4>
                    {viewingProduct.note ? (
                      <div className="text-gray-700 bg-amber-50 border border-amber-200 rounded-md p-3 text-sm shadow-sm leading-relaxed">
                        {viewingProduct.note}
                      </div>
                    ) : (
                      <p className="text-gray-500 italic text-sm">No specific note provided.</p>
                    )}
                  </div>
                </div>
