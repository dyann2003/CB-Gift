...
export default function ManageCatalog() {
  ...
  async function fetchCategories() {
    try {
      const res = await fetch("https://localhost:7015/api/Categories/public");
  ...
  async function fetchProducts() {
    try {
      ...
      const url = `https://localhost:7015/api/Product/filter?searchTerm=${searchTerm}&category=${category}&status=${status}&page=${page}&pageSize=${itemsPerPage}`;
  ...
  const handleViewDetails = async (product) => {
    try {
      const res = await fetch(
        `https://localhost:7015/api/Product/${product.id}`
      );
      const data = await res.json();
  ...
  const handleSoftDelete = async (id) => {
    try {
      const res = await fetch(
        `https://localhost:7015/api/Product/hidden/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleRestore = async (id) => {
    try {
      const res = await fetch(
        `https://localhost:7015/api/Product/restore/${id}`,
        {
          method: "PATCH",
        }
      );
