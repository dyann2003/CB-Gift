...
export default function ManageCategory() {
  ...
  const fetchCategories = async () => {
    try {
      const url = `https://localhost:7015/api/Categories/filter?searchTerm=${searchTerm}&page=${page}&pageSize=${itemsPerPage}`;
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const res = await fetch("https://localhost:7015/api/Categories", {
        method: "POST",
  ...
  const handleEditCategory = async (id) => {
    try {
      const res = await fetch(`https://localhost:7015/api/Categories/${id}`);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(
        `https://localhost:7015/api/Categories/${editingId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleRestoreCategory = async (id) => {
    try {
      const res = await fetch(
        `https://localhost:7015/api/Categories/${id}/status`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
