            ...
            <li key={item.id} className="px-4">
              <button
                ...
                className={`w-full flex items-center p-3 my-1 rounded-lg text-left text-sm font-medium transition-colors duration-200 ${
                  statusFilter === item.id ? "bg-blue-500 text-white shadow" : "text-gray-600 hover:bg-gray-100"
                }`}
              >
...
export default function StaffProductionStatusPage() {
  ...
  useEffect(() => {
    ...
    fetchProductionData()
  }, [selectedCategory, selectedDate, statusFilter, updateTrigger])
  ...
  const handleGroupSubmitted = async () => {
    setIsGrouping(true)
    try {
      const response = await fetch("https://localhost:7015/api/Plan/group-submitted", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      // Trigger a re-fetch of the data to show the updated groups
      setUpdateTrigger((prev) => prev + 1)
      ...
      <main className="flex-1 overflow-y-auto bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 flex items-center justify-between">
            ...
            <button
              onClick={handleGroupSubmitted}
              disabled={isGrouping}
              className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed font-medium shadow-sm"
            >
              {isGrouping ? "Grouping..." : "Group Submitted"}
            </button>
          </div>
                      ...
                      >
                        <div className="bg-yellow-300 p-3">
                          <h3 className="font-bold text-yellow-800">
                            {new Date(dateGroup.groupDate).toLocaleDateString("en-US", {
                              month: "long",
                              day: "2-digit",
                              year: "numeric",
                            })}
                          ...
                          <table className="min-w-full text-sm">
                            <thead className="bg-gray-50">
                              <tr>
                                ...
                                <th className="p-3 text-center font-semibold text-gray-600" colSpan="4">
                                  Documents
                                </th>
