...
export default function ManageInvoicePage() {
    ...
    const fetchReceivables = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://localhost:7015/api/invoices/seller-receivables?${params.toString()}`,
          {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        const response = await fetch(
          `https://localhost:7015/api/invoices/seller-receivables?page=1&pageSize=10000`, // Lấy tất cả
          { credentials: "include" }
        );
        const data = await response.json();
